...
contract AcbToken is IERC20, Ownable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    address payable public fundAddress;
    address  public initlpadress;
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 holdTokenTotal = holdToken.totalSupply() - holdToken.balanceOf(initlpadress);
    ...
    function setInitlpadress(address addr) external onlyOwner {
        initlpadress= addr;
    }

     function setEnableOffTrade(bool val) external onlyOwner {
        enableOffTrade= val;
    }

     function setrewardToken(address val) external onlyOwner {
        rewardToken= val;
    }
    function setrcurrency(address val) external onlyOwner {
        currency= val;
    }
}